...
| `--storage-provider <provider>`     | —       | Select a Files SDK provider such as `vercel-blob`, `s3`, `r2`, or `gcs`.             |
| `--storage-config <json>`           | —       | Pass non-secret JSON options to the selected adapter. Credentials use env vars.      |
