...
3. The static wrappers (`installed-tools.ts`, `tool-renderer-registry.ts`) automatically pick up the new entries while tightly coupled tools remain under `tools/platform`
