...
`PersistentChatRuntimes` renders controllers outside the route tree, so
...
| `PersistentChatRuntimes` | Keeps runtime controllers mounted outside route transitions |
...
6. The persistent runtime's `ChatSync` sends the primary request and receives stream data.
...
- the first request is sent by the persistent runtime controller
