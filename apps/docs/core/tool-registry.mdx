...
Tools that need project-level storage configuration must declare `"projectRequirements": ["storage"]` in both their item manifest and registry index entry. `chatjs create` reads the index before scaffolding and prompts for storage when any selected installable tool declares this requirement. A top-level tool's index entry must include its complete project requirements, including requirements introduced by its registry dependencies.
