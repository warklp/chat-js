...
5. Updates the CLI-managed registry index so the tool is available to the model and renderer
...
`tools/chatjs/index.ts` is the only CLI-managed registry index. You should not edit it manually.
