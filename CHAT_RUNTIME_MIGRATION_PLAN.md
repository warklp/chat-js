...
- home/project-home routes create or reuse a provisional runtime keyed by the
  autogenerated chat id on route entry
- draft and provisional chats are the same runtime state; submit only attaches
  pending submission data to the existing runtime
- `ChatSystem` no longer mounts `ChatSync`; runtime controllers are mounted only
  by `MountedChatRuntimes`
- runtime-owned data stream state is shared with the visible chat tree so
  artifact/tool stream handlers can remain UI-scoped
- idle historical and draft chats are runtime-backed once visited; cleanup is
  deferred to Phase 6
...
- Historical chats now get runtimes on route entry; cleanup/eviction timing
  remains open.
- Whether `ChatSync` should be renamed to `ChatRuntimeController`.
- How background runtime errors should notify the user.
- Data stream state now belongs to the runtime entry; visible UI handlers still
  process artifact/tool stream parts.

## Recommended Next Step

Continue Phase 5 with browser verification for two simultaneous streams: open
an existing chat, start a stream, navigate away, start another stream, then
return to confirm both stores and controllers remain independent.
